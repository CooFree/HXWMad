...
-(void)drawRect:(CGRect)rect
{
    ...
    //画原点
    originPoint = CGPointMake(tailWidth, self.height - tailHeight);
    NSString *zero = @"0";
    [zero drawAtPoint:CGPointMake(originPoint.x - CalcTextWidth([UIFont systemFontOfSize:titleFont], @"0"), originPoint.y) withAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:titleFont], NSFontAttributeName,xyColor,NSForegroundColorAttributeName, nil]];
    //画X轴标示文字
    ...
    if(isShowLump)
    {
        ...
        for (int i = 0; i < _lineAry.count; i++) {
            NSArray *lineDetailAry = self.lineAry[i];
            UIColor *color = [(UIColor *)_lineColorAry[i] colorWithAlphaComponent:.12];
            //原点
            CGContextMoveToPoint(context, originPoint.x + widthX, originPoint.y);
            for (int m = 0; m < lineDetailAry.count; m++) {
                CGFloat x = tailWidth + widthX * (m + 1);
                CGFloat y = self.height - tailHeight - [lineDetailAry[m] floatValue]/maxY * widthY * _yAry.count;
                //线上面的点
                CGContextAddLineToPoint(context, x, y);
                //坐标系右下方的店
                if (m == lineDetailAry.count - 1) {
                    CGContextAddLineToPoint(context, x, self.height - tailHeight);
                }
            }
            //回到原点(即左下角的店)
